    ...
    <section className="panel drafts-panel" aria-label="Drafts">
      ...
      <div className="drafts-layout">
        ...
        {selected ? <div ref={editorRef} className="draft-editor" onKeyDown={handleKeys}>
          ...
          {selected.from && <div className="composer-account">From {selected.from}</div>}
          <textarea ref={bodyInput} value={selected.body} onPaste={(event) => { if (event.clipboardData.files.length || event.clipboardData.types.includes("text/uri-list") && event.clipboardData.getData("text/uri-list").startsWith("file://")) { event.preventDefault(); setError("HEY CLI cannot add files to an existing saved draft yet. Use HEY, or attach them in a new message or reply."); } }} onDragOver={(event) => { if (event.dataTransfer.types.includes("Files")) event.preventDefault(); }} onDrop={(event) => { if (event.dataTransfer.files.length) { event.preventDefault(); setError("HEY CLI cannot add files to an existing saved draft yet. Use HEY, or attach them in a new message or reply."); } }} onChange={(event) => setSelected({ ...selected, body: event.target.value })} />
