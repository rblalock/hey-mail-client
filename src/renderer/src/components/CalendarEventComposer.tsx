...
export default function CalendarEventComposer({ calendars, startsOn, event, initial, onClose, onSaved }: Props) {
  ...
  const [applyTo, setApplyTo] = useState<"current" | "future">("current");
  const [allowPlainNotes, setAllowPlainNotes] = useState(false);
  ...
  const update = useMemo<CalendarEventUpdateRequest | undefined>(() => {
    ...
    if (event.occurrenceId) {
      next.id = event.seriesId ?? event.occurrenceId.split("_")[0]!;
      next.occurrenceId = event.occurrenceId; next.applyTo = applyTo;
      if (allowPlainNotes) next.allowPlainNotes = true;
    }
    ...
    if ((!event.occurrenceId || applyTo === "future") && repeat && repeat !== "current") next.repeat = repeat;
    ...
    return next;
  }, [allDay, applyTo, allowPlainNotes, circle, countdown, countdownUnit, end.date, end.time, endDate, endTime, event, invites, link, location, notes, originalInvites, originalReminders, originalRepeat, reminders, repeat, repeatUntil, request.title, start.date, start.time, startDate, startTime, timeZone]);
  const selectedCalendar = calendars.find((calendar) => calendar.id === calendarId);
  const hasChanges = update ? Object.keys(update).some((key) => !["id", "lookupDate", "occurrenceId", "applyTo", "allowPlainNotes"].includes(key)) && (!event?.occurrenceId || applyTo !== "future" || Boolean(update.repeat)) : true;
  ...
  const invalidCountdown = countdown !== "" && (!Number.isInteger(Number(countdown)) || Number(countdown) < (editing ? 0 : 1) || Number(countdown) > 30);
    ...
    <section ref={dialogRef} className="calendar-composer" role="dialog" aria-modal="true" aria-label={reviewing ? "Review event invitations" : titleText} onKeyDown={(event) => {
      ...
      {reviewing ? <div className="calendar-event-review">
        ...
        {writableCalendars.length === 0 || editing && !event?.calendar.writable ? <div className="calendar-composer-unavailable"><strong>This event is read-only</strong><p>Subscribed and external calendars must be edited in their source calendar.</p></div> : <>
          ...
          {event?.occurrenceId && <><label className="calendar-field"><span>Apply changes to</span><select value={applyTo} onChange={(change) => setApplyTo(change.target.value as "current" | "future")}><option value="current">This occurrence only</option><option value="future">This and future occurrences</option></select></label>{applyTo === "future" && <p className="calendar-invite-warning">Choose the new repeat schedule and end date in More options. A blank end date repeats indefinitely.</p>}<label className="calendar-check"><input type="checkbox" checked={allowPlainNotes} onChange={(change) => setAllowPlainNotes(change.target.checked)} /><span>Allow notes to become plain text if HEY requires it (formatting may be lost)</span></label></>}
          ...
          <details className="calendar-more-fields" open={moreOpen} onToggle={(toggle) => setMoreOpen(toggle.currentTarget.open)}><summary>More options</summary><div className="calendar-form-row"><label className="calendar-field"><span>Reminders</span><input value={reminders} onChange={(event) => setReminders(event.target.value)} placeholder="10m, 1h, 1d" /></label><label className="calendar-field"><span>Repeat</span><select disabled={Boolean(event?.occurrenceId) && applyTo === "current"} value={repeat} onChange={(event) => { const value = event.target.value as RepeatChoice; setRepeat(value); if (!value || value === "current") setRepeatUntil(""); }}>{event?.recurring && <option value="current">Keep current schedule</option>}{repeatOptions.filter((option) => !event?.recurring || option.value !== "").map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label></div>{(!event?.occurrenceId || applyTo === "future") && repeat && repeat !== "current" && <label className="calendar-field"><span>Repeat until (optional)</span><input type="date" min={startDate} value={repeatUntil} onChange={(event) => setRepeatUntil(event.target.value)} /></label>}{editing && <div className="calendar-form-row"><label className="calendar-field"><span>Countdown (optional)</span><input type="number" min="0" max="30" value={countdown} onChange={(event) => setCountdown(event.target.value)} placeholder="Number" /></label><label className="calendar-field"><span>Countdown unit</span><select value={countdownUnit} onChange={(event) => setCountdownUnit(event.target.value as typeof countdownUnit)}><option value="days">Days</option><option value="weeks">Weeks</option><option value="months">Months</option></select></label></div>}</details>
          {invalidInvites.length > 0 && <p id={inviteErrorId} className="composer-error" role="alert">Choose a contact or enter a complete email address for {invalidInvites.join(", ")}.</p>}{clearedReminders && <p className="composer-error">Removing every reminder is not supported by HEY CLI yet. Change the reminder or use HEY.</p>}{invalidSchedule && <p className="composer-error">The event must end after it starts.</p>}{invalidCountdown && <p className="composer-error">Countdowns must be a whole number from 0 to 30. Use 0 to remove a countdown.</p>}
        </>}
      </div>}
